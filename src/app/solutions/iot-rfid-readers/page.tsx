...
import { 
  ArrowRight, Radio, Cpu, Wifi, Usb,
  Warehouse, ShoppingCart, Factory, Truck
} from 'lucide-react';

// Reader Modules
const readerModules = [
  {
    name: 'UHF Reader Modules',
    frequency: '860-960 MHz',
    interface: 'UART / SPI / I2C',
    range: 'Up to 10m',
    use: 'OEM integration, custom readers',
    color: 'bg-ids-blue'
  },
  {
    name: 'HF/NFC Modules',
    ...
    interface: 'UART / USB',
    range: 'Up to 10cm',
    use: 'Access control, payment terminals',
    color: 'bg-ids-orange'
  },
  {
    name: 'LF Reader Modules',
    frequency: '125/134 kHz',
    interface: 'UART / Wiegand',
    range: 'Up to 10cm',
    use: 'Animal tracking, legacy systems',
    color: 'bg-green-500'
  }
];
...
// Key Features
const keyFeatures = [
  { name: 'Multi-Protocol', desc: 'ISO 18000-6C, ISO 14443A/B, ISO 15693' },
  { name: 'Low Power', desc: 'Battery-powered handheld integration' },
  { name: 'Fast Read Rate', desc: 'Up to 400 tags/second bulk reading' },
  { name: 'SDK Provided', desc: 'C, Python, Java libraries included' }
];

// Integration Use Cases
const useCases = [
  { icon: Warehouse, name: 'Inventory Systems', desc: 'Embed in custom scanning stations' },
  { icon: ShoppingCart, name: 'POS Terminals', desc: 'Contactless payment integration' },
  { icon: Factory, name: 'Production Lines', desc: 'Automated tracking checkpoints' },
  { icon: Truck, name: 'Fleet Management', desc: 'Vehicle-mounted readers' }
];
...
export default function IotRfidReadersPage() {
  const pageTitle = 'IoT RFID Readers';
          ...
          <main className="w-full lg:w-3/4 order-1 lg:order-2">
            ...
            >
              ...
              >
                ...
                >
                  RFID Reader<br />
                  <span className="text-ids-orange">Modules</span>
                </motion.h1>
                <motion.p 
                ...
                >
                  Embeddable RFID reader modules for OEM integration. LF, HF, NFC, and UHF options with SDK support for custom development.
                </motion.p>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
            ...
            >
              <motion.div variants={fadeInUp} className="flex items-center gap-3 mb-4">
                ...
                <span className="text-ids-orange font-medium">Modules</span>
              </motion.div>
                ...
                {readerModules.map((mod, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-4">
                      <div className={`w-3 h-10 rounded-full ${mod.color}`}></div>
                      <h3 className="font-bold">{mod.name}</h3>
                    </div>
                    <div className="space-y-2 text-sm mb-3">
                      <div className="flex justify-between text-gray-400">
                        <span>Frequency</span>
                        <span className="text-white">{mod.frequency}</span>
                      </div>
                      <div className="flex justify-between text-gray-400">
                        <span>Interface</span>
                        <span className="text-white">{mod.interface}</span>
                      </div>
                      <div className="flex justify-between text-gray-400">
                        <span>Range</span>
                        <span className="text-ids-orange">{mod.range}</span>
                      </div>
                    </div>
            ...
            >
              ...
              <motion.div variants={staggerContainer} className="grid md:grid-cols-2 gap-5">
                {keyFeatures.map((feature, i) => (
                  ...
                  >
                    <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-ids-blue/20 to-green-500/10 flex items-center justify-center shrink-0">
                      <Cpu className="text-ids-blue" size={20} />
                    </div>
                    <div>
                ...
                {useCases.map((useCase, i) => (
                  ...
                  >
                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-green-500/20 to-ids-yellow/10 flex items-center justify-center shrink-0">
                      <useCase.icon className="text-green-400" size={22} />
                    </div>
                    <div>
                      <h3 className="font-bold mb-0.5">{useCase.name}</h3>
            ...
            >
              ...
              <div className="relative z-10">
                <h3 className="text-2xl md:text-3xl font-bold mb-4">Build Custom Readers</h3>
                <p className="text-gray-300 mb-8 max-w-2xl">
                  Our OEM modules come with complete SDK, technical documentation, and integration support for your custom RFID solution.
                </p>
                <div className="flex flex-wrap gap-4">
                  ...
                  >
                    Get Module Samples <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                  </Link>
