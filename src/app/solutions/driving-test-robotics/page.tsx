...
import { 
  ArrowRight, Car, Navigation, Target, Timer,
  CheckCircle2, AlertTriangle, BarChart3, Shield
} from 'lucide-react';
...
const testModules = [
  {
    ...
    sensors: ['Ultrasonic array', 'IR beams'],
    scoring: 'Distance, angle, time',
    color: 'bg-ids-blue'
  },
  {
    name: 'Slope & Ramp',
    sensors: ['Inclinometer', 'Position sensors'],
    scoring: 'Rollback, stop accuracy',
    color: 'bg-ids-orange'
  },
  {
    name: 'Lane Discipline',
    sensors: ['RFID track markers', 'Cameras'],
    scoring: 'Lane deviation, speed',
    color: 'bg-green-500'
  }
];
...
const systemFeatures = [
  { icon: Target, name: 'Precision Scoring', desc: 'Sensor-based measurement accuracy' },
  { icon: Timer, name: 'Real-time Results', desc: 'Instant pass/fail determination' },
  { icon: BarChart3, name: 'Analytics Dashboard', desc: 'Test statistics & reports' },
  { icon: Shield, name: 'Tamper-Proof', desc: 'Fraud prevention built-in' }
];

// Implementation Benefits
const benefits = [
  { icon: CheckCircle2, text: 'Eliminates human bias in evaluation' },
  { icon: CheckCircle2, text: 'Standardized testing across all RTOs' },
  { icon: CheckCircle2, text: 'Reduces corruption opportunities' },
  { icon: CheckCircle2, text: 'Faster throughput, shorter queues' }
];
...
export default function DrivingTestRoboticsPage() {
  const pageTitle = 'Driving Test Robotics';
          ...
          <main className="w-full lg:w-3/4 order-1 lg:order-2">
            ...
            >
              ...
              >
                ...
                >
                  Automated<br />
                  <span className="text-ids-orange">Driving Test</span> Robotics
                </motion.h1>
                <motion.p 
                ...
                >
                  Sensor-based automated driving test tracks for RTOs. Unbiased evaluation of parallel parking, slope test, and lane discipline.
                </motion.p>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  ...
                  >
                    Get Proposal <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                  </Link>
                </motion.div>
              </motion.div>
            </motion.div>
            ...
            >
              <motion.div variants={fadeInUp} className="flex items-center gap-3 mb-4">
                ...
                <span className="text-ids-orange font-medium">Test Tracks</span>
              </motion.div>
                ...
                {testModules.map((module, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3 mb-4">
                      <div className={`w-3 h-10 rounded-full ${module.color}`}></div>
                      <h3 className="font-bold">{module.name}</h3>
                    </div>
                    <div className="flex flex-wrap gap-1 mb-3">
                      {module.sensors.map((sensor, j) => (
                        <span key={j} className="text-xs px-2 py-1 bg-white/5 rounded text-gray-300">{sensor}</span>
                      ))}
                    </div>
                    <p className="text-xs text-gray-400">Scoring: {module.scoring}</p>
                  </motion.div>
                ))}
              </motion.div>
            </motion.section>
            ...
            >
              ...
              <motion.div variants={staggerContainer} className="grid md:grid-cols-2 gap-4">
                {benefits.map((benefit, i) => (
                  ...
                  >
                    <benefit.icon className="text-green-400 shrink-0" size={20} />
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <p className="text-gray-300 mb-8 max-w-2xl">
                  Implement transparent, automated driving tests that meet government standards and eliminate manual evaluation.
                </p>
                <div className="flex flex-wrap gap-4">
                  ...
                  <Link href="/solutions/automated-test-tracks" className="bg-transparent border border-white/20 text-white font-bold px-8 py-3 rounded-full hover:bg-white/10 transition-colors">
                    Test Tracks
                  </Link>
                </div>
              </div>
