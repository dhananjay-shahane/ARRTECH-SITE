...
import { 
  ArrowRight, Zap, Battery, Gauge, Shield,
  Settings, Thermometer, Factory, Server
} from 'lucide-react';
...
const powerTypes = [
  {
    name: 'DIN Rail SMPS',
    specs: ['12V/24V/48V', '10W-480W', 'Industrial mount'],
    use: 'Panel automation, PLCs',
    color: 'bg-ids-orange'
  },
  {
    name: 'Enclosed Power Supplies',
    specs: ['Single/Multi output', 'PFC built-in', 'Fan cooled'],
    use: 'LED displays, kiosks',
    color: 'bg-ids-blue'
  },
  {
    name: 'UPS & Battery Backup',
    specs: ['Offline/Online', 'Lead-acid/Li-ion', '1kVA-10kVA'],
    use: 'Access control, servers',
    color: 'bg-green-500'
  }
];
...
const specifications = [
  { icon: Zap, label: 'Efficiency', value: 'Up to 94%' },
  { icon: Thermometer, label: 'Operating Temp', value: '-25°C to +70°C' },
  { icon: Shield, label: 'Protection', value: 'OVP, OCP, SCP' },
  { icon: Gauge, label: 'Regulation', value: '±1% Load/Line' }
];

// Applications
const applications = [
  { icon: Factory, name: 'Industrial Automation', desc: 'PLCs, sensors, motor drives' },
  { icon: Server, name: 'Access Control', desc: 'Door controllers, readers' },
  { icon: Battery, name: 'Backup Systems', desc: 'Emergency power, UPS' }
];
...
export default function PowerSupplyPage() {
  const pageTitle = 'Power Supply Solutions';
          ...
          <main className="w-full lg:w-3/4 order-1 lg:order-2">
            ...
            >
              ...
              >
                ...
                >
                  Industrial SMPS, DIN rail power supplies, and UPS systems. Reliable power for automation, access control, and IoT infrastructure.
                </motion.p>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
              ...
              <motion.div variants={staggerContainer} className="grid md:grid-cols-3 gap-6">
                {powerTypes.map((type, i) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-400">For: {type.use}</p>
                  </motion.div>
                ))}
              </motion.div>
            </motion.section>
            ...
            >
              <motion.div variants={fadeInUp} className="flex items-center gap-3 mb-4">
                ...
                <span className="text-ids-blue font-medium">Specs</span>
              </motion.div>
              <motion.h2 variants={fadeInUp} className="text-3xl md:text-4xl font-bold mb-10">
              ...
              <motion.div variants={staggerContainer} className="grid md:grid-cols-4 gap-5">
                {specifications.map((spec, i) => (
                  ...
                  >
                    <div className="w-10 h-10 mx-auto rounded-lg bg-gradient-to-br from-ids-blue/20 to-green-500/10 flex items-center justify-center mb-3">
                      <spec.icon className="text-ids-blue" size={20} />
                    </div>
                    <h3 className="font-bold text-sm mb-1">{spec.label}</h3>
                    <p className="text-ids-orange text-sm">{spec.value}</p>
                  </motion.div>
                ))}
              </motion.div>
            </motion.section>
            ...
            >
              ...
              <div className="relative z-10">
                <h3 className="text-2xl md:text-3xl font-bold mb-4">Power Your Project</h3>
                <p className="text-gray-300 mb-8 max-w-2xl">
                  From single units to bulk orders, we supply industrial-grade power solutions with technical support.
                </p>
