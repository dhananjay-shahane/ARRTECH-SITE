...
import { 
  ArrowRight, Sparkles, Shield, Eye, Layers, 
  CreditCard, Building, GraduationCap, BadgeCheck, Zap
} from 'lucide-react';

// Laminate Types
const laminateTypes = [
  {
    name: 'Generic Holographic',
    desc: 'Standard holographic pattern overlay for basic security',
    security: 'Basic',
    applications: ['Staff IDs', 'Membership cards', 'Access badges'],
    color: 'bg-ids-blue'
  },
  {
    name: 'Custom Holographic',
    desc: 'Your logo and design embedded in holographic foil',
    security: 'High',
    applications: ['Corporate cards', 'Government IDs', 'Student cards'],
    color: 'bg-ids-orange'
  },
  {
    name: 'Security Laminate',
    desc: 'Micro-text, guilloche, UV features in laminate',
    security: 'Very High',
    applications: ['National IDs', 'Licenses', 'Passports'],
    color: 'bg-green-500'
  }
];
...
const securityFeatures = [
  { icon: Eye, name: 'Optically Variable', desc: 'Colors shift with viewing angle' },
  { icon: Shield, name: 'Tamper Evident', desc: 'Shows VOID if peeled' },
  { icon: Sparkles, name: 'Laser Microtext', desc: 'Invisible to naked eye' },
  { icon: Zap, name: 'UV Reactive', desc: 'Glows under blacklight' }
];

// Application Cards
const applicationCards = [
  { icon: CreditCard, name: 'National ID Cards', desc: 'Government-grade security laminates' },
  { icon: Building, name: 'Corporate Badges', desc: 'Branded holographic overlays' },
  { icon: GraduationCap, name: 'Student IDs', desc: 'Durable anti-counterfeit protection' },
  { icon: BadgeCheck, name: 'Licenses & Permits', desc: 'Tamper-evident verification' }
];
...
export default function HolographicLaminationPage() {
  const pageTitle = 'Holographic Lamination';
          ...
          <main className="w-full lg:w-3/4 order-1 lg:order-2">
              ...
              >
                ...
                >
                  <span className="inline-flex items-center gap-2 px-4 py-2 bg-ids-orange/20 backdrop-blur-sm rounded-full text-ids-orange text-sm font-medium mb-4">
                    <Sparkles size={16} /> Card Security
                  </span>
                </motion.div>
                <motion.h1 
                ...
                >
                  Holographic<br />
                  <span className="text-ids-orange">Lamination</span>
                </motion.h1>
                <motion.p 
                ...
                >
                  Add an extra layer of security and authenticity to your ID cards. Holographic overlaminates prevent counterfeiting and extend card life.
                </motion.p>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  ...
                  >
                    Get Sample Kit <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                  </Link>
                </motion.div>
              </motion.div>
            </motion.div>

            {/* Section 1: Laminate Types */}
            <motion.section 
              initial="hidden"
              whileInView="visible"
            ...
            >
              <motion.div variants={fadeInUp} className="flex items-center gap-3 mb-4">
                ...
                <span className="text-ids-orange font-medium">Options</span>
              </motion.div>
                ...
                {laminateTypes.map((type, i) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-xs px-2 py-1 bg-white/10 rounded-full text-gray-300">{type.security} Security</span>
                    </div>
                    <h3 className="font-bold text-lg mb-2">{type.name}</h3>
                    <p className="text-gray-400 text-sm mb-4">{type.desc}</p>
                    <div className="flex flex-wrap gap-1">
                      {type.applications.map((app, j) => (
                        <span key={j} className="text-xs text-gray-400">• {app}</span>
                      ))}
                    </div>
                  </motion.div>
                ))}
              ...
              <motion.div variants={staggerContainer} className="grid md:grid-cols-2 gap-5">
                {securityFeatures.map((feature, i) => (
                  <motion.div 
                    key={i}
                    variants={fadeInUp}
                    className="flex items-start gap-4 p-5 rounded-xl bg-white/5 border border-white/5 hover:border-ids-blue/40 transition-all group"
                  >
                    <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-ids-blue/20 to-green-500/10 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
            ...
            >
              ...
              <div className="relative z-10">
                <h3 className="text-2xl md:text-3xl font-bold mb-4">Secure Your Cards</h3>
                <p className="text-gray-300 mb-8 max-w-2xl">
                  From generic holographic overlays to fully custom security laminates, we provide solutions for every security level and budget.
                </p>
                <div className="flex flex-wrap gap-4">
                  ...
                  <Link href="/solutions/card-printing" className="bg-transparent border border-white/20 text-white font-bold px-8 py-3 rounded-full hover:bg-white/10 transition-colors">
                    Card Printing
                  </Link>
                </div>
              </div>
