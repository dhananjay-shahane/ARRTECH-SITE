...
import { 
  ArrowRight, Monitor, ToggleRight, Gauge, Lightbulb,
  Factory, Building, DoorOpen, Grid3X3
} from 'lucide-react';
...
const productCategories = [
  {
    ...
    products: ['7-segment LED', 'LCD modules', 'OLED panels'],
    interface: 'I2C, SPI, RS485',
    color: 'bg-ids-blue'
  },
  {
    name: 'Relay Modules',
    products: ['1/2/4/8 channel', 'Solid state', 'DIN rail'],
    interface: 'Digital I/O, Modbus',
    color: 'bg-ids-orange'
  },
  {
    name: 'Interface Components',
    products: ['Level shifters', 'Optocouplers', 'Signal converters'],
...
// Use Cases
const useCases = [
  { icon: Factory, name: 'Industrial Automation', desc: 'Machine status, process control' },
  { icon: Building, name: 'Building Systems', desc: 'HVAC, lighting, elevators' },
  { icon: DoorOpen, name: 'Access Control', desc: 'Door locks, turnstile control' },
  { icon: Grid3X3, name: 'Queue & Display', desc: 'Token counters, signage' }
];
...
export default function DisplaysRelaysPage() {
  const pageTitle = 'Displays & Relay Systems';
          ...
          <main className="w-full lg:w-3/4 order-1 lg:order-2">
              ...
              >
                ...
                >
                  <span className="inline-flex items-center gap-2 px-4 py-2 bg-ids-blue/20 backdrop-blur-sm rounded-full text-ids-blue text-sm font-medium mb-4">
                    <Monitor size={16} /> Interface Components
                  </span>
                </motion.div>
                <motion.h1 
                ...
                >
                  Displays &<br />
                  <span className="text-ids-blue">Relay Systems</span>
                </motion.h1>
                <motion.p 
                ...
                >
                  Industrial displays, relay modules, and interface components for automation, access control, and IoT applications.
                </motion.p>
                <motion.div
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-1 mb-3">
                      {cat.products.map((prod, j) => (
                        <span key={j} className="text-xs px-2 py-1 bg-white/5 rounded text-gray-300">{prod}</span>
                      ))}
                    </div>
                    <p className="text-xs text-gray-400">Interface: {cat.interface}</p>
                  </motion.div>
                ))}
              </motion.div>
            </motion.section>
                ...
                {useCases.map((use, i) => (
                  ...
                  >
                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-ids-orange/20 to-green-500/10 flex items-center justify-center shrink-0">
                      <use.icon className="text-ids-orange" size={22} />
                    </div>
                    <div>
            ...
            >
              ...
              <div className="relative z-10">
                <h3 className="text-2xl md:text-3xl font-bold mb-4">Complete Your System</h3>
                <p className="text-gray-300 mb-8 max-w-2xl">
                  Displays, relays, and interface components to connect your automation projects.
                </p>
                <div className="flex flex-wrap gap-4">
                  ...
                  <Link href="/solutions/power-supply" className="bg-transparent border border-white/20 text-white font-bold px-8 py-3 rounded-full hover:bg-white/10 transition-colors">
                    Power Supplies
                  </Link>
                </div>
              </div>
